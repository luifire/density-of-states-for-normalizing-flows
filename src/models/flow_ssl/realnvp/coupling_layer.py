...
from flow_ssl.fully_connected.fully_connected_net import FullyNeuralNetwork
...
class CouplingLayerBase(nn.Module):
    ...
    """ implementation = 'nice', 'real_vp', 'rnvp'"""
    counter = 0
    def __init__(self, implementation='rnvp'):
        super(CouplingLayerBase, self).__init__()
    ...
    def constant_rnvp_scale_normalizer(self, s):
        # controlled: I
        s_flat = s.view(s.size(0), -1)
        log_sum = s_flat.sum(-1)
        # half of the inputs are ignored, hence 2 * N
        normalizer = log_sum / (s_flat.shape[-1] // 2)
        full_normalizer = normalizer.unsqueeze(1).repeat(1, s_flat.shape[1]).reshape(s.shape)
        
        masked_normalizer = self.mask.mask_st_output(full_normalizer, full_normalizer)[0]
        s = s - masked_normalizer
    ...
    def forward(self, x, sldj=None, reverse=True):
        ...
        if self.implementation == 'nice':
            # inserted by David
            x_change = x_change + t
            self._logdet = torch.zeros(size=(len(x),), device=x.device)
        else:
            if self.implementation == 'real_vp':
                s = self.constant_rnvp_scale_normalizer(s)

            # normal procedure
            exp_s = s.exp()
            if not torch.isfinite(exp_s).all():
                print('max scale ', s.max(), ' max scale.exp() ', s.max().exp())
                raise RuntimeError('Scale factor has NaN or Inf entries')

            #if CouplingLayerBase.counter % 1000 == 0:
            #    print(exp_s.detach()[0])
            #    print("s:", exp_s.detach().var().cpu().numpy(), exp_s.detach().min().cpu().numpy(), exp_s.detach().max().cpu().numpy())
            #CouplingLayerBase.counter += 1
            ...
            leave as I am yet to afraid of very changes due to instability
            """
            ...
            #print(self._logdet.max(), self._logdet.mean())

            #if not torch.isfinite(x_change).all():
            #    print(t, exp_s)
            #    raise RuntimeError('x_change has NaN or Inf entries')
    ...
    def inverse(self, y):
        ...
        if self.implementation == 'nice':
            # inserted by David
            x_change = x_change - t
            self._logdet = torch.zeros(size=(len(y),), device=y.device)
        else:
            if self.implementation == 'real_vp':
                s = self.constant_rnvp_scale_normalizer(s)
...
class CouplingLayer(CouplingLayerBase):
    ...
    def __init__(self, in_channels, mid_channels, num_blocks, mask, init_zeros=False, st_type='resnet',
                 use_batch_norm=True, img_width=28, skip=True, latent_dim=100, nice_implementation=False):
        super(CouplingLayer, self).__init__(nice_implementation)
        ...
        if st_type == 'resnet_ae':
            ...
            input_size = img_width**2 * in_channels
            self.st_net = FullyNeuralNetwork(input_size, input_size*4, 2*input_size)

        # Learnable scale for s
        #print('####################!!!!!!!!!!!!!!!!!!!!')
        #print('big change here')
        self.rescale = nn.utils.weight_norm(Rescale(in_channels))
        #self.rescale = nn.utils.weight_norm(Rescale(input_size))
...
class Rescale(nn.Module):
    ...
    """
    def __init__(self, num_channels, one_dimensional=False):
        ...
        #print('####################!!!!!!!!!!!!!!!!!!!!')
        #print('big change below4')

    def forward(self, x):
        #x = self.weight.reshape(x.shape[1:]) * x
        x = self.weight * x
        return x
