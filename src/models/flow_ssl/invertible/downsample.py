...
# David:
class EvenSpreadOut(nn.Module):
    """Just turns data from [0,1] to [-0.5, 0.5]"""
    def __init__(self):
        super().__init__()
...
def i_logits(z, cnstr=0.9):
    ...
    spl = F.softplus(torch.Tensor([1 - cnstr]).log() - torch.Tensor([cnstr]).log()).to(z.device)
    # before
    logdet = (F.softplus(z) + F.softplus(-z) - spl).sum(3).sum(2).sum(1) + math.log(255 / 256) * sum_numel
    return z, logdet
...
class iLogits(nn.Module):
    def __init__(self, cnstr=0.90):  # should change to .95
        super().__init__()
        self.cnstr = cnstr
        warnings.warn('Ilogits Activated!!!!!!!!!!!')
    ...
    def forward(self, x):
        ...
        """
        z, self._logdet = i_logits(z, self.cnstr)
        # after
        #self._logdet = (F.softplus(z) + F.softplus(-z) - spl).sum(3).sum(2).sum(1)
        return z
