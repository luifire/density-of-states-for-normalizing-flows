...
def loglik_bpd(model, x):
    ...
    # David: Note that this is also the mean (like for nf)
    # except that the 1/N is in x.shape.numel() (because it is over the
    # batch dimension as well)
    return - model.log_prob(x).sum() / (math.log(2) * x.shape.numel())


def loglik_bpd_per_sample(model, x):
    """Compute the log-likelihood in bits per dim."""
    return - model.log_prob(x) / (math.log(2) * np.prod(x.shape[1:]))
...
def dataset_iwbo_nats(model, data_loader, k, device, double=False, kbs=None, verbose=True):
    with torch.no_grad():
        ...
        for i, x in enumerate(data_loader):
            ...
            count += len(x)
            #if verbose:
            print('{}/{}'.format(i+1, len(data_loader)), nats/count)
    return nats / count
...
def dataset_iwbo_bpd(model, data_loader, k, device, double=False, kbs=None, verbose=True):
    ...
    with torch.no_grad():
        ...
        for i, x in enumerate(data_loader):
            ...
            print('a')
            #if verbose:
            print('{}/{}'.format(i+1, len(data_loader)), bpd/count)
    return bpd / count
