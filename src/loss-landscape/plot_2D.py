...
def plot_2d_contour(surf_file, surf_name='train_loss', vmin=0.1, vmax=10, vmax_factor=None, vlevel=0.5, show=False):
    ...
    if vmax_factor:
        Z_finite = Z[finite_Z]
        vmax = Z_finite.min() + np.abs(Z_finite.min()) * vmax_factor
        vmax = min(vmax, Z_finite.max())
        #vmax = Z[finite_Z].max()

    Z[np.logical_not(finite_Z)] = vmax * 0.99999
    #############
    ...
    CS = plt.contour(X, Y, Z, cmap='summer', levels=np.arange(Z.min(), vmax, vlevel))
    ...
    CS = plt.contourf(X, Y, Z, cmap='summer', levels=np.arange(Z.min(), vmax, vlevel))
    ...
    sns_plot = sns.heatmap(Z, cmap='viridis', cbar=True,
                           xticklabels=False, yticklabels=False, vmax=vmax)
