...
################################################################################
def get_weights(net, args):
    """ Extract parameters from net, and return a list of tensors"""
    # return [p.data for p in net.parameters()]
    return [p.data for p in get_filtered_parameters(net, args)]


def get_filtered_parameters(net, args):
    lst = list()
    if args.weight_filter == 'filter_scaling':
        ...
        return lst
    elif args.weight_filter == 'per_st_net':
        #body.3.1.
        all_weights = []
        weights_of_part = []
        current_part = [None, None, None]

        for name, weights in net.named_parameters():
            part = name.split('.')
            #'body.4.0.module1.rescale.weight_v',
            #'body.6.trainable_part'
            if part[1] != current_part[1] or part[2] != current_part[2]:
                if len(weights_of_part) > 0:
                    all_weights.append(torch.cat(weights_of_part))
                weights_of_part = []
                current_part = part

            weights_of_part.append(weights.flatten())
        return all_weights
    else:
        return [weights for _, weights in net.named_parameters()]
...
def set_weights(net, weights, directions=None, step=None, args=None):
    ...
    if directions is None:
        ...
        for (p, w) in zip(get_filtered_parameters(net, args), weights):
        ...
        for i, (p, w, d) in enumerate(zip(get_filtered_parameters(net, args), weights, changes)):
            if len(p) != len(w) or len(p) != len(d) or len(w) != len(d):
                print(i)
...
def create_random_direction(net, args, dir_type='weights', ignore='biasbn', norm='filter'):
    """
    ...
    # random direction
    if dir_type == 'weights':
        weights = get_weights(net, args)  # a list of parameters.
...
def setup_direction(args, dir_file, net):
    ...
    if not args.dir_file:
        ...
        if args.model_file2:
            ...
            xdirection = create_random_direction(net, args, args.dir_type, args.xignore, args.xnorm)
        xdirection = [x.cpu() for x in xdirection]
        ...
        if args.y:
            if args.same_dir:
                ...
                ydirection = create_random_direction(net, args, args.dir_type, args.yignore, args.ynorm)
            ydirection = [y.cpu() for y in ydirection]
...
def name_direction_file(args, net_config):
    """ Name the direction file that stores the random directions. """
    dir_file = 'F:/loss_plot'

    filter = '_' + args.xnorm if args.xnorm != 'filter' else ''

    dir_file = path.join(dir_file, args.checkpoint,
                         f'[{args.xmin},{args.xmax},{args.xnum}]_weight_filter={args.weight_filter}{filter}')
    os.makedirs(dir_file, exist_ok=True)

    file_name = f'{net_config.flow}_{args.data_mode}_{net_config.dimensions}_l.h5'
    dir_file = path.join(dir_file, file_name)
